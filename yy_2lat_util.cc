    ...
    Oxs_ComputeEnergyExtraData& oceed)
{
  ...
  if(energies.size() == 0) {
    ...
    if(oced1.energy_accum) {
      for(OC_INDEX i=0; i<size; ++i) {
        (*(oced1.energy_accum))[i] = 0.0;
      }
    }
    if(oced1.H_accum) {
      for(OC_INDEX i=0; i<size; ++i) {
        (*(oced1.H_accum))[i] = ThreeVector(0.0,0.0,0.0);
      }
    }
    if(oced1.mxH_accum) {
      for(OC_INDEX i=0; i<size; ++i) {
        (*(oced1.mxH_accum))[i] = ThreeVector(0.0,0.0,0.0);
      }
    }
    if(oceed.mxHxm) {
      for(OC_INDEX i=0; i<size; ++i) {
  ...
  foo.ocedt.scratch_energy = oced1.scratch_energy;
  foo.ocedt.scratch_H      = oced1.scratch_H;
  foo.ocedt.energy_accum   = oced1.energy_accum;
  foo.ocedt.H_accum        = oced1.H_accum;
  foo.ocedt.mxH_accum      = oced1.mxH_accum;
  ...
  for(vector<Oxs_Energy*>::const_iterator ncit = nonchunk.begin();
    ...
    Oxs_ComputeEnergyData term_oced1(state);
    ...
    ++(eterm.calc_count);
    eterm.ComputeEnergy(state,term_oced1);

/** FILE: yy_2latdemag-threaded.cc            -*-Mode: c++-*-
 *
 * Threaded version of yy_2latdemag.cc (q.v.).
 *
 * Average H demag field across rectangular cells. Develooped based upon
 * Oxs_Demag.
 *
 * This code uses the Oxs_FFT3v classes to perform DFFTs of the import 
 * magnetization ThreeVectors as the Oxs_Demag class. The function 
 * definitions are provided in yy_2latdemag.cc and yy_2latdemag-threaded.cc
 * and one of them is compiled depending on the setting of the compiler 
 * macro OOMMF_THREADS.
 *
...
#include "yy_2latdemag.h"  // Includes definition of OOMMF_THREADS macro
...
class _YY_2LatDemagJobControl {
public:

  _YY_2LatDemagJobControl() : imax(0),big_block_limit(0),big_blocksize(0),
...
void _YY_2LatDemagJobControl::Init
(OC_INDEX i_imax,
...
void _YY_2LatDemagJobControl::ClaimJob(OC_INDEX& istart,OC_INDEX& istop)
{
...
////////////////////////////////////////////////////////////////////////
// YY_2LatDemag::Oxs_FFTLocker provides thread-specific instance of FFT
// objects.

YY_2LatDemag::Oxs_FFTLocker::Oxs_FFTLocker
(const YY_2LatDemag::Oxs_FFTLocker_Info& info)
...
YY_2LatDemag::Oxs_FFTLocker::~Oxs_FFTLocker()
{
  ...
  if(A_copy)        Oc_FreeThreadLocal(A_copy,
                       A_copy_size*sizeof(YY_2LatDemag::A_coefs));
}

////////////////////////////////////////////////////////////////////////
// YY_2LatDemag Constructor
YY_2LatDemag::YY_2LatDemag(
...
void YY_2LatDemag::ReleaseMemory() const
{ // Conceptually const
  if(A!=0) { delete[] A; A=0; }
...
void YY_2LatDemag::FillCoefficientArrays(const Oxs_Mesh* genmesh) const
{ // This routine is conceptually const.

  // YY_2LatDemag requires a rectangular mesh
  ...
  if(pmesh==NULL) {
    ...
    if(xperiodic+yperiodic+zperiodic>2) {
      String msg=String("Periodic mesh ")
        + String(genmesh->InstanceName())
        + String("is 3D periodic, which is not supported by YY_2LatDemag."
    ...
    if(xperiodic+yperiodic+zperiodic>1) {
      String msg=String("Periodic mesh ")
        + String(genmesh->InstanceName())
      + String("is 2D periodic, which is not supported by YY_2LatDemag"
               " at this time.");
      throw Oxs_ExtError(this,msg.c_str());
    }
  }
  ...
  // up it own copies using keyword "YY_2LatDemagFFTObject" to
  ...
  //         YY_2LatDemag::IncrementPreconditioner
  if(xperiodic) {
    OXS_FFT_REAL_TYPE fft_scaling = -1 *
      fftx.GetScaling() * ffty.GetScaling() * fftz.GetScaling();
    /// Note: Since H = -N*M, and by convention with the rest of this
...
class _YY_2LatDemagFFTxThread : public Oxs_ThreadRunObj {
public:
  ...
  YY_2LatDemag::Oxs_FFTLocker_Info locker_info;
...
void _YY_2LatDemagFFTxThread::Cmd(int threadnumber, void* /* data */)
{
  ...
  if(!fftx) {
    ...
    if(!foo) {
      ...
      foo = new YY_2LatDemag::Oxs_FFTLocker(locker_info);
      local_locker.AddItem(locker_info.name,foo);
    }
    fftx = &(dynamic_cast<YY_2LatDemag::Oxs_FFTLocker*>(foo)->fftx);
    if(!fftx) {
      Oxs_ThreadError::SetError(String("Error in"
         "_YY_2LatDemagFFTxThread::Cmd(): fftx downcast failed."));
      return;
    }
  }
...
class _YY_2LatDemagiFFTxDotThread : public Oxs_ThreadRunObj {
public:
  ...
  YY_2LatDemag::Oxs_FFTLocker_Info locker_info;
  YY_2LatDemag::Oxs_FFTLocker* locker;
...
Oxs_Mutex _YY_2LatDemagiFFTxDotThread::result_mutex;
Nb_Xpfloat _YY_2LatDemagiFFTxDotThread::energy_sum(0.0);

void _YY_2LatDemagiFFTxDotThread::Cmd(int threadnumber, void* /* data */)
{
  // Thread local storage
  if(!locker) {
    Oxs_ThreadMapDataObject* foo = local_locker.GetItem(locker_info.name);
    ...
    locker = dynamic_cast<YY_2LatDemag::Oxs_FFTLocker*>(foo);
    if(!locker) {
      Oxs_ThreadError::SetError(String("Error in"
         "_YY_2LatDemagFFTxDotThread::Cmd(): locker downcast failed."));
      return;
    }
  }
...
class _YY_2LatDemagFFTyThread : public Oxs_ThreadRunObj {
public:
  OXS_FFT_REAL_TYPE* carr;
  ...
  static _YY_2LatDemagJobControl job_control;
...
_YY_2LatDemagJobControl _YY_2LatDemagFFTyThread::job_control;

void _YY_2LatDemagFFTyThread::Cmd(int /* threadnumber */, void* /* data */)
{
  // Thread local storage
  if(!ffty) {
    Oxs_ThreadMapDataObject* foo = local_locker.GetItem(locker_info.name);
    ...
    ffty = &(dynamic_cast<YY_2LatDemag::Oxs_FFTLocker*>(foo)->ffty);
    if(!ffty) {
      Oxs_ThreadError::SetError(String("Error in"
         "_YY_2LatDemagFFTyThread::Cmd(): ffty downcast failed."));
      return;
    }
  }
...
class _YY_2LatDemagFFTyConvolveThread : public Oxs_ThreadRunObj {
  // *ONLY* for use when cdimz==1
public:
  OXS_FFT_REAL_TYPE* Hxfrm;
  const YY_2LatDemag::A_coefs* A;
  ...
  _YY_2LatDemagFFTyConvolveThread()
...
_YY_2LatDemagJobControl _YY_2LatDemagFFTyConvolveThread::job_control;

void _YY_2LatDemagFFTyConvolveThread::Cmd(int /* threadnumber */, void* /* data */)
{
  // Thread local storage
  if(!locker) {
    Oxs_ThreadMapDataObject* foo = local_locker.GetItem(locker_info.name);
    ...
    if(!locker) {
      Oxs_ThreadError::SetError(String("Error in"
         "_YY_2LatDemagFFTyConvolveThread::Cmd(): locker downcast failed."));
      return;
    }
  }
    ...
    for(OC_INDEX ix=istart;ix<istop;ix+=embed_block_size) {
      ...
      { // j==0
        for(OC_INDEX i=ix;i<ix_end;++i) {
          const YY_2LatDemag::A_coefs& Aref = A[i];
          {
      ...
      for(j=1;j<cdimy/2;++j) {
        ...
        for(OC_INDEX i=ix;i<ix_end;++i) {
          const YY_2LatDemag::A_coefs& Aref = A[ajindex+i];
          { // j>0
...
class _YY_2LatDemagFFTzConvolveThread : public Oxs_ThreadRunObj {
public:
  OXS_FFT_REAL_TYPE* Hxfrm;
  const YY_2LatDemag::A_coefs* A;
  ...
  _YY_2LatDemagFFTzConvolveThread()
    : Hxfrm(0), A(0), locker(0),
      thread_count(0),
...
_YY_2LatDemagJobControl _YY_2LatDemagFFTzConvolveThread::job_control;

void _YY_2LatDemagFFTzConvolveThread::Cmd(int /* threadnumber */, void* /* data */)
{
  // Thread local storage
  if(!locker) {
    Oxs_ThreadMapDataObject* foo = local_locker.GetItem(locker_info.name);
    ...
    if(!locker) {
      Oxs_ThreadError::SetError(String("Error in"
         "_YY_2LatDemagFFTzConvolveThread::Cmd(): locker downcast failed."));
      return;
    }
  }
  ...
  if(locker->A_copy == NULL) {
    ...
    locker->A_copy = static_cast<YY_2LatDemag::A_coefs*>
      (Oc_AllocThreadLocal(asize*sizeof(YY_2LatDemag::A_coefs)));
    memcpy(locker->A_copy,A,asize*sizeof(YY_2LatDemag::A_coefs));
  }
  const YY_2LatDemag::A_coefs* const Acopy = locker->A_copy;
    ...
    for(j=jstart;j<jstop;++j) {
      ...
      for(OC_INDEX m=0;m<cdimx;m+=embed_block_size) {
        ...
        for(k=0;k<adimz;++k) {
          ...
          for(i=m;i<istop;++i) {
            const YY_2LatDemag::A_coefs& Aref = Acopy[akindex+i];
...
// asdf //////////////////////////////
#if USE_FFT_YZ_CONVOLVE
class _YY_2LatDemagFFTyzConvolveThread : public Oxs_ThreadRunObj {
public:
  const YY_2LatDemag::A_coefs* A;
  OXS_FFT_REAL_TYPE* carr;
  ...
  _YY_2LatDemagFFTyzConvolveThread()
    : A(0), carr(0), locker(0),
...
void _YY_2LatDemagFFTyzConvolveThread::Cmd(int thread_number, void* /* data */)
{
  // Thread local storage
  if(!locker) {
    Oxs_ThreadMapDataObject* foo = local_locker.GetItem(locker_info.name);
...
#endif
        for(i=0;i<ispan;++i) {
          const YY_2LatDemag::A_coefs& Aref = Acopy[Aindex+i];
          { // j>=0, k>=0
 ...
 ) const
{
  ...
  fftxforwardtime.Start();
    ...
    vector<_YY_2LatDemagFFTxThread> fftx_thread;
    ...
    for(ithread=0;ithread<MaxThreadCount;++ithread) {
      ...
      fftx_thread[ithread].direction = _YY_2LatDemagFFTxThread::FORWARD;
  ...
  if(cdimz<2) {
    ...
    {
      OXS_FFT_REAL_TYPE *Hxfrm = Hxfrm_base.GetArrBase();
      vector<_YY_2LatDemagFFTyConvolveThread> ffty_thread;
      ffty_thread.resize(MaxThreadCount);

      _YY_2LatDemagFFTyConvolveThread::job_control.Init(cdimx,MaxThreadCount,1);
    ...
    {
      OXS_FFT_REAL_TYPE *Hxfrm = Hxfrm_base.GetArrBase();
      vector<_YY_2LatDemagFFTyThread> ffty_thread;
      ffty_thread.resize(MaxThreadCount);

      _YY_2LatDemagFFTyThread::job_control.Init(cdimx*ODTV_VECSIZE*rdimz,
                                             MaxThreadCount,16);
      ...
      for(ithread=0;ithread<MaxThreadCount;++ithread) {
        ...
        ffty_thread[ithread].direction = _YY_2LatDemagFFTyThread::FORWARD;
        if(ithread>0) threadtree.Launch(ffty_thread[ithread],0);
      }
      threadtree.LaunchRoot(ffty_thread[0],0);
    }
...
#endif // REPORT_TIME
    {
      ...
      // Multi-thread
      vector<_YY_2LatDemagFFTzConvolveThread> fftzconv;
      fftzconv.resize(MaxThreadCount);

      _YY_2LatDemagFFTzConvolveThread::job_control.Init(adimy,MaxThreadCount,1);
...
#endif // REPORT_TIME
    {
      ...
      for(ithread=0;ithread<MaxThreadCount;++ithread) {
        ...
        ffty_thread[ithread].k_dim = rdimz;
        ffty_thread[ithread].i_dim = cdimx*ODTV_VECSIZE;
        ffty_thread[ithread].direction = _YY_2LatDemagFFTyThread::INVERSE;
        if(ithread>0) threadtree.Launch(ffty_thread[ithread],0);
      }
      threadtree.LaunchRoot(ffty_thread[0],0);
    }
...
#endif // REPORT_TIME
    {
      ...
      // Multi-thread
      vector<_YY_2LatDemagFFTyzConvolveThread> fftyzconv;
...

  {
    ...
    vector<_YY_2LatDemagiFFTxDotThread> fftx_thread;
    fftx_thread.resize(MaxThreadCount);

    _YY_2LatDemagiFFTxDotThread::Init(); // Zeros energy_sum
    for(ithread=0;ithread<MaxThreadCount;++ithread) {
      fftx_thread[ithread].carr = Hxfrm;
    ...
    Nb_Xpfloat tempsum;
    _YY_2LatDemagiFFTxDotThread::result_mutex.Lock();
    tempsum = _YY_2LatDemagiFFTxDotThread::energy_sum;
    _YY_2LatDemagiFFTxDotThread::result_mutex.Unlock();
    oced.energy_sum

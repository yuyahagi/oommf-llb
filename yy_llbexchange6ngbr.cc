/* FILE: yy_llbexchange6ngbr.cc            -*-Mode: c++-*-
 *
 * 6 neighbor exchange energy on rectangular mesh, derived from Oxs_Energy
 * class. Developed based on YY_LLBExchange6Ngbr.
 *
 */
...
#include "yy_llbexchange6ngbr.h"

OC_USE_STRING;

// Oxs_Ext registration support
OXS_EXT_REGISTER(YY_LLBExchange6Ngbr);
...
// Constructor
YY_LLBExchange6Ngbr::YY_LLBExchange6Ngbr(
    ...
    coef_size(0), coef(NULL), mesh_id(0)
{
  ...
  try {
    ...
    maxspinangle_output.Setup(this,InstanceName(),"Max Spin Ang","deg",1,
                              &YY_LLBExchange6Ngbr::UpdateDerivedOutputs);
...
OC_BOOL YY_LLBExchange6Ngbr::Init()
{
  mesh_id = 0;
  region_id.Release();
...
void YY_LLBExchange6Ngbr::CalcEnergyA
 ...
 ) const
{
  ...
  if(mesh==NULL) {
    String msg =
      ...
      + String("\") to YY_LLBExchange6Ngbr::GetEnergyA()"
...
void YY_LLBExchange6Ngbr::ComputeEnergyChunkFinalize
(const Oxs_SimState& state,
 ...
 int number_of_threads) const
{
  ...
  if(state.GetDerivedData(msa_name,dummy_value)) {
    ...
    maxangleset.Send(revision_info,OC_STRINGIFY(__LINE__),
                     "Programming error?"
                     " YY_LLBExchange6Ngbr max spin angle set twice.");
#endif
    ...
    if(fabs(diff*sum)>8*OC_REAL8_EPSILON) {
      ...
      Oc_Snprintf(errbuf,sizeof(errbuf),
                  "Programming error:"
                  " YY_LLBExchange6Ngbr max spin angle set to"
...
void YY_LLBExchange6Ngbr::ComputeEnergyChunk
(const Oxs_SimState& state,
 Oxs_ComputeEnergyDataThreaded& ocedt,
 Oxs_ComputeEnergyDataThreadedAux& ocedtaux,
 OC_INDEX node_start,
      ...
      if(mesh_id != state.mesh->Id() || !atlaskey.SameState()) {
        ...
        if(mesh_id !=  state.mesh->Id() || !atlaskey.SameState()) {
          ...
          Oxs_ThreadPrintf(stderr,"Invalid condition in"
                           " YY_LLBExchange6Ngbr::ComputeEnergyChunk(),"
                           " thread number %d\n",threadnumber);
        }
        if(thread_control.count>0) {
      ...
      try {
        ...
        for(OC_INDEX i=0;i<size;i++) {
          ...
          if((region_id[i] = atlas->GetRegionId(location))<0) {
            String msg = String("Import mesh to YY_LLBExchange6Ngbr::GetEnergy()"
                                " routine of object ")
              + String(InstanceName())
...
void YY_LLBExchange6Ngbr::UpdateDerivedOutputs(const Oxs_SimState& state)
{
...
OC_INT4m
YY_LLBExchange6Ngbr::IncrementPreconditioner(PreconditionerData& pcd)
{
      ...
      if((region_id[i] = atlas->GetRegionId(location))<0) {
        String msg
          = String("Import mesh to"
                   " YY_LLBExchange6Ngbr::IncrementPreconditioner()"
                   " routine of object ")
          + String(InstanceName())
          + String(" has points outside atlas ")
          + String(atlas->InstanceName());

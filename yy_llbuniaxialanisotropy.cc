/* FILE: yy_llbuuanisotropy.cc            -*-Mode: c++-*-
 *
 * Uniform Uniaxial Anisotropy, derived from Oxs_Energy class. Developed
 * based on Oxs_UniaxialAnisotropy.
 *
 */
...
#include "yy_llbuniaxialanisotropy.h"

OC_USE_STRING;

// Oxs_Ext registration support
OXS_EXT_REGISTER(YY_LLBUniaxialAnisotropy);

/* End includes */


// Constructor
YY_LLBUniaxialAnisotropy::YY_LLBUniaxialAnisotropy(
...
OC_BOOL YY_LLBUniaxialAnisotropy::Init()
{
  mesh_id = 0;
  K1.Release();
  Ha.Release();
...
void
YY_LLBUniaxialAnisotropy::StageRequestCount
(unsigned int& min,
...
void
YY_LLBUniaxialAnisotropy::GetMultiplier
...
void YY_LLBUniaxialAnisotropy::RectIntegEnergy
 ...
 ) const
{
  ...
  for(OC_INDEX i=node_start;i<node_stop;++i) {
    ...
    // Spin polarization cubed (Ms/Ms0)^3
    OC_REAL8m mi = (*state.Ms)[i]*(*state.Ms0_inverse)[i];
    OC_REAL8m mi3 = mi*mi*mi;
...
void YY_LLBUniaxialAnisotropy::ComputeEnergyChunk
(const Oxs_SimState& state,
 Oxs_ComputeEnergyDataThreaded& ocedt,
 Oxs_ComputeEnergyDataThreadedAux& ocedtaux,
 OC_INDEX node_start,OC_INDEX node_stop,
 ...
 ) const
{
      ...
      if(mesh_id != state.mesh->Id()) {
        ...
        if(mesh_id !=  state.mesh->Id()) {
          ...
          Oxs_ThreadPrintf(stderr,"Invalid condition in"
                           " YY_LLBUniaxialAnisotropy::ComputeEnergyChunk(),"
                           " thread number %d\n",threadnumber);
        }
        if(thread_control.count>0) {
  ...
  if(mesh==NULL) {
    throw Oxs_ExtError(this,
          "Import mesh to YY_LLBUniaxialAnisotropy::GetEnergy()"
...
OC_INT4m
YY_LLBUniaxialAnisotropy::IncrementPreconditioner(PreconditionerData& pcd)
{

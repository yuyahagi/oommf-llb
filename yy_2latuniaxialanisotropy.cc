/* FILE: yy_2latuuanisotropy.cc            -*-Mode: c++-*-
 *
...
#include "yy_2latuniaxialanisotropy.h"
#include "energy.h"		// Needed to make MSVC++ 5 happy

OC_USE_STRING;

// Oxs_Ext registration support
OXS_EXT_REGISTER(YY_2LatUniaxialAnisotropy);

/* End includes */


// Constructor
YY_2LatUniaxialAnisotropy::YY_2LatUniaxialAnisotropy(
...
OC_BOOL YY_2LatUniaxialAnisotropy::Init()
{
  mesh_id = 0;
  K11.Release();
  Ha1.Release();
...
void
YY_2LatUniaxialAnisotropy::StageRequestCount
(unsigned int& min,
...
void
YY_2LatUniaxialAnisotropy::GetMultiplier
...
void YY_2LatUniaxialAnisotropy::RectIntegEnergy
...
void YY_2LatUniaxialAnisotropy::ComputeEnergyChunk
(const Oxs_SimState& state,
 Oxs_ComputeEnergyDataThreaded& ocedt,
 Oxs_ComputeEnergyDataThreadedAux& ocedtaux,
 OC_INDEX node_start,OC_INDEX node_stop,
 ...
 ) const
{
      ...
      if(mesh_id != state.mesh->Id()) {
        ...
        if(mesh_id !=  state.mesh->Id()) {
          ...
          Oxs_ThreadPrintf(stderr,"Invalid condition in"
                           " YY_2LatUniaxialAnisotropy::ComputeEnergyChunk(),"
                           " thread number %d\n",threadnumber);
        }
        if(thread_control.count>0) {
  ...
  if(mesh==NULL) {
    throw Oxs_ExtError(this,
          "Import mesh to YY_2LatUniaxialAnisotropy::GetEnergy()"
...
OC_INT4m
YY_2LatUniaxialAnisotropy::IncrementPreconditioner(PreconditionerData& pcd)
{
